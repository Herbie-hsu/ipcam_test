...
{
	print "usage:	
	hw_setting_convert.pl ver(none/e1) ./sn986xx sdk
	hw_setting_convert.pl ver(none/e1) ./sn986xx sdk -f filename
	hw_setting_convert.pl ver(none/e1) ./sn986xx keil
	hw_setting_convert.pl ver(none/e1) ./sn986xx jlink
	hw_setting_convert.pl ver(none/e1) ./sn986xx quake -h 372\n";
}
...
if (@ARGV == 3){
	($version,$serial_name,$which_type) = @ARGV;
}else{
	($version,$serial_name,$which_type,$which_flag,$which_value) = @ARGV;
}
...
{				
  ...
  ###sdk -f
	if(($which_type =~ /^sdk$/i)&&($which_flag =~ /-f/)){
		  ...
		  #print $target_file."\n";
			&parse_to_asm_file($which_file,$target_file);		
	}else{
	  my $create_dir = "";
			...
			###sdk
				...
				while(my $file = readdir(DIR))
				{
					if ($file =~ /^[^\.]/){
						...
						#print "[$target_file]\n";
						&parse_to_asm_file($this_file,$target_file);
					}
				}			
			}
			###quake -h
...
#			}
			...
			if (($addr eq $addr_3) || ($addr eq $addr_4) || ($addr eq $addr_5) || ($addr eq $addr_6) || ($addr eq $addr_7)){
				$addr =~ tr/[A-Z]/[a-z]/;
...
#   die "Missing output file name.\n" unless $out;
   ...
   foreach(@script){	
		if (/WM32/){
			...
			if (($line[1] =~/0x903/) ||($line[1] =~/0x98A/) ||($line[1] =~/0x98000000/) ||($line[1] =~/0x98000004/)){
				print OUT "w $reg,$dat\n";
			}
		}
		elsif (/DELAY/){
