...
int set_hw_timer_alarm (int timer_id, unsigned int ms, timer_handler_t handler, unsigned long arg)
{
	...
	if (timer_id == 0)
	{
		...
		writel (APB_CLK, TIMER2_LOAD);
	}
	else //if(timer_id == 1)
	{
		//timer 3
		writel (value, TIMER3_COUNT);
		writel (APB_CLK, TIMER3_LOAD);
	}
...
int enable_hw_timer_measure_mode (int timer_id)
{
	...
	{
		//timer 2
		writel (APB_CLK, TIMER2_COUNT);
		writel (APB_CLK, TIMER2_LOAD);
	}
	else //if(timer_id == 1)
	{
		//timer 3
		writel (APB_CLK, TIMER3_COUNT);
		writel (APB_CLK, TIMER3_LOAD);
	}
...
static void __init snx_timer_setup (void)
{
	...
	writel ((TM1ENABLE | TM1OFENABLE | TM3ENABLE | TM3UPDOWN) & (~(TM1CLOCK | TM1UPDOWN)), TIMER_CTRL);
}
