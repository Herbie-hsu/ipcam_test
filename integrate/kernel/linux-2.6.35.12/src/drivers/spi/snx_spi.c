...
static void snx_spi_initialsetup(struct snx_spi *hw, unsigned long hz)
{
	...
	if (hw->bpw < 16)
		...
		ssp_set_fifo_threshold(hw, 16, 1);
...
//just for Lookback mode test
	...
	ssp_enable (hw, 0);
	if (hw->bpw == 8)
		ssp_write_data (hw, hw_txbyte(hw, 0));
	else if (hw->bpw == 16)
		ssp_write_data (hw, hw_tx2byte(hw, 0));
	else	
		ssp_write_data (hw, hw_tx4byte(hw, 0));
