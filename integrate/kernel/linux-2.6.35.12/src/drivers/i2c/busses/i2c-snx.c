...
static void snx_i2c_reset(struct snx_i2c *i2c)
{
	...
	if (i2c->i2cclk > 10 * I2C_STANDARD_CLK)
		i2c->i2cclk = I2C_STANDARD_CLK;
	
//	cnt = (APB_CLK/i2c->i2cclk - 1 + gsr )/2;
	cnt = (APB_CLK / i2c->i2cclk - gsr) / 2 - 2;
...
static int snx_i2c_do_xfer(struct snx_i2c *i2c, struct i2c_msg *msg, int num)
{
	...
	if (ret) {
		...
		printk("snx_i2c: timeout waiting for bus free\n");
    goto out;
	}
...
{
  *((volatile unsigned *)(BASE_I2C + I2C_GPIO_OFFSET)) = 0;    
  *((volatile unsigned *)(BASE_I2C + I2C_GPIO_OFFSET)) = 0;
}
