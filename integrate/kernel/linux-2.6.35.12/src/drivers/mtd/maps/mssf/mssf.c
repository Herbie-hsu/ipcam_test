...
#define	MAX_READY_WAIT_COUNT	1000000 //100000
...
#define AMIC_MIXC_16M	0   //ID {0xc2,0x20,0x18}
#define MX25L12835E	1   //ID {0xc2,0x20,0x19}
struct sf_id id_mixc_arry[] = {
			{0xc2,0x20,0x18}, /* MIXC-16 serial flash 16M */
...
static int wait_till_msrdy_ready(struct mssf *flash)
{
	int count;
	DBG_ME ("%s:%d: ---->\n", __func__, __LINE__);
...
static int read_sr(struct mssf *flash)
{
	...
	if(wait_till_msrdy_ready(flash))
...
int sf_checkid (struct sf_id id, int typenum)
{
	...
	switch (typenum) {
		case SF_MXIC_AMIC:
			...
			if (index == AMIC_MIXC_16M || index == MX25L12835E) {
		...
		case SF_GD:
			...
			if (index == GD25Q128C || index == GD25Q16B) {
				snx_sf_data->block_size = 32 * 1024; //32k
				snx_sf_data->sector_size = 4 * 1024; //4k
...
#if 0
//add by zhangchao test
	unsigned int i=0,len=0,testerr=0,start=0,n=0;
	char buf[640] = {0};
	char retbuf[640] = {0};
	if(flash->sf_inst.typenum != SF_EN)
		erase_block(flash, 0);
	...
	for(n=0;n<20;n++){
		start = 640 * n;
		testerr = 0;
		printk("***********test the start addr is %dk**********\n",start/640);
		snx_sf_write_dma(&sf_flash->mtd, start, 640, &len, buf);
		snx_sf_read_dma(&sf_flash->mtd,	start, 640, &len, retbuf);
		for(i=0; i<640; i++){
			if(buf[i] != retbuf[i]){
				printk("the buf[%d]=0x%x, retbuf[%d]=0x%x \n",i, buf[i], i, retbuf[i]);
				testerr++;
			}
		}
		if(err)
