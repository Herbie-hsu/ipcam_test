...
int audio_cap_format_check( snx_audio_stream_conf_t *stream)
{
	...
	switch (stream->format) {
		...
		case SND_PCM_FORMAT_U16:
			...
			strcpy(stream->dev_name, "snx_audio_pcm");
			break;
		case SND_PCM_FORMAT_A_LAW:
			stream->frame_number = 80;
			strcpy(stream->dev_name, "snx_audio_alaw");
			break;
		case SND_PCM_FORMAT_MU_LAW:
			stream->frame_number = 80;
			strcpy(stream->dev_name, "snx_audio_mulaw");
			break;
		case SND_PCM_FORMAT_G726_16:
			stream->frame_number = 80;
			strcpy(stream->dev_name, "snx_audio_g726");
			break;
		...
		case SND_PCM_FORMAT_G722:
			stream->frame_number = 80;
			strcpy(stream->dev_name, "snx_audio_g722");
			break;
		default:
...
/*  Alsa conf modification is needed */
		case SND_PCM_FORMAT_A_LAW:
			stream->frame_number = 160;
			strcpy(stream->dev_name, "snx_audio_alaw_pb");
			break;
		case SND_PCM_FORMAT_MU_LAW:
			stream->frame_number = 160;
			strcpy(stream->dev_name, "snx_audio_mulaw_pb");
			break;
		case SND_PCM_FORMAT_G726_16:
			stream->frame_number = 160;
			strcpy(stream->dev_name, "snx_audio_g726_pb");
			break;
		case SND_PCM_FORMAT_G726_24:
			stream->frame_number = 160;
			strcpy(stream->dev_name, "snx_audio_g726_pb");
...
void snx_audio_pb_flow (void *arg)
{
    ...
    while (!feof(fd)) {
		...
		while ( read_frame ) {
			rc = snd_pcm_writei(handle, buffer+ptr, (read_frame_size - cached_frame_size));
