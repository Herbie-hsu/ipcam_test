...
char rootfs_end_address_1[ROOTFS_R__LENGTH] = "empty";
char rootfs_end_address_2[ROOTFS_R__LENGTH] = "empty";
char *rootfs_flag_1 = "rootfs-r";
char *rootfs_flag_2= "rootfs-r-2";
unsigned int rootfs_r_len = 0;
...
int main(int argc, char **argv)
{
	...
	FILE *con_fp, *find_rootfs_r_size;
	char *t;
	char *delim = " ";
	char *p;
  ...
  //read flash layout file
	...
	while(fgets(line,READ_LENGTH,con_fp)) {
		...
		while(t != NULL){
			...
			if(section_num == 2){
				...
				if(strcmp(type,rootfs_flag_1) == 0){
					memcpy(rootfs_end_address_1,address_end,ROOTFS_R__LENGTH);
					rootfs_end_address_1[ROOTFS_R__LENGTH - 1] = '\0';
				}
				if(strcmp(type,rootfs_flag_2) == 0){
					memcpy(rootfs_end_address_2,address_end,ROOTFS_R__LENGTH);
					rootfs_end_address_2[ROOTFS_R__LENGTH - 1] = '\0';
				}
			}
			if(section_num == 3){
				sprintf(file,"%s",t);
				if(file[strlen(file) - 1] == 0xd) file[strlen(file) - 1] = '\0';
				//get the size of rootfs-r
				if((strcmp(type,rootfs_flag_1) == 0)||(strcmp(type,rootfs_flag_2) == 0)){
					if(!(find_rootfs_r_size=fopen(file,"rb"))) {
    						printf("Can not open '%s'!\n",file);
    						return -1;
					} 
					fseek(find_rootfs_r_size, 0, SEEK_END);  
					 if((rootfs_r_len=ftell(find_rootfs_r_size))==-1){
      						printf("Sorry! Can not calculate '%s' which larger than 2 GB!\n",file);    
      						fclose(find_rootfs_r_size);
     						return -1;
					} 
					 fclose(find_rootfs_r_size);
				}
	...
	*/
	...
	/**
   	 ...
   	 */
		if(deal_with_insertdata(user_conf,out_file,rootfs_end_address_1,rootfs_end_address_2,rootfs_r_len)){
