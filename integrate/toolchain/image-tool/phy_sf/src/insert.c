...
int insert_data(char *filename, char *insert_start, char *insert_length, char *insert_value)
{
	...
	if(length == 1){
		value_char = (unsigned char) strtoll(insert_value, NULL, 0);
		fwrite(&value_char,length,1,stream);
	}
	else if(length == 2){
		value_short = (unsigned short) strtoll(insert_value, NULL, 0);
		fwrite(&value_short,length,1,stream);
	}
	else if(length == 4){
		value_int = (unsigned int) strtoll(insert_value, NULL, 0);
		fwrite(&value_int,length,1,stream);
	}
	else{
